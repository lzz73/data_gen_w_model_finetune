<template>
  <div class="page-container">
    ...
    <div v-if="currentStep === 1" class="content-card">
      ...
      <el-alert
        v-if="localCount > 0 && !trainConfig.baseModel"
        title="检测到本地模型，请从下方列表选择或搜索"
        type="info"
        show-icon
        :closable="false"
...
const availableDatasets = ref<TrainingDatasetItem[]>([])
const gpuList = ref<any[]>([])
const selectedGpus = ref<any[]>([])
...
const trainConfig = reactive<{
  ...
  do_eval: boolean; val_size: string; eval_dataset: string
  eval_steps: number; save_steps: number
  logging_steps: number; save_total_limit: number; flash_attn: string
  output_dir: string
  resume_from_checkpoint: boolean; checkpoint_path: string
  gpu: string; num_gpus: number
}>({
  mode: 'sft',
...
const onGpuSelect = (selection: any[]) => {
...
const loadDatasets = async () => {
  try {
    availableDatasets.value = await trainingApi.listTrainingDatasets()
...
const loadGpuInfo = async () => {
  ...
  try {
    const data = await trainingApi.getGpuInfo()
    if (data) gpuList.value = data
    // 自动选中第一个可用 GPU
...
const submitTask = async () => {
  ...
  if (!trainConfig.baseModel || !trainConfig.baseModel.trim()) {
  ...
  try {
    ...
    // 前端用 baseModel（驼峰），后端用 base_model（下划线）
    payload.base_model = payload.baseModel
    delete payload.baseModel
    await trainingApi.createTask(payload)
...
const loadModels = async () => {
  try {
    ...
    const data = (res as any)?.models || res
    const models = Array.isArray(data) ? data : []
    if (models.length > 0) {
      baseModels.value = models.map((m: any) => ({
        value: m.local && m.local_path ? m.local_path : (m.huggingface || m.name),
        label: m.name,
        hf: m.local && m.local_path ? m.local_path : (m.huggingface || ''),
        local: m.local || false,
      }))
      localCount.value = baseModels.value.filter(m => m.local).length
      // 自动选第一个本地模型
      if (!trainConfig.baseModel) {
        const firstLocal = baseModels.value.find(m => m.local)
        if (firstLocal) trainConfig.baseModel = firstLocal.value
      }
    }
