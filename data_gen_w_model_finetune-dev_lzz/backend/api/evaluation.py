...
_eval_tasks: dict = {}
_eval_file = ROOT / "eval_tasks.json"
...
@router.get("/tasks")
def api_eval_tasks():
    return {"code": 0, "data": sorted(_eval_tasks.values(), key=lambda t: t["created_at"], reverse=True)}
...
def api_eval_run(config: EvalConfig):
    ...
    return {"code": 0, "data": {"task_id": tid}, "message": "评估已启动"}
...
def _run_eval(task_id: str, cfg: dict):
    ...
    1. 通过 llamafactory data loader 加载数据集（正确处理模板）
    2. 批量推理
    3. 计算 BLEU/ROUGE
    """
    ...
    try:
        ...
        data_args.dataset_dir = str(ROOT / "data")
        ...
        data_dir = data_args.dataset_dir or str(ROOT / "data")
