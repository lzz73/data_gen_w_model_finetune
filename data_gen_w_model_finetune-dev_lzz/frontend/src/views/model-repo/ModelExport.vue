...
import { exportModel } from '@/api/training'
...
const doExport = async () => {
  ...
  try {
    ...
    if (res.code === 0 && res.data?.job_id) {
      exportResultDir.value = res.data.export_dir
      // 轮询进度
      pollTimer = setInterval(async () => {
        try {
          const statusRes = await get<any>(`/training/export/status/${res.data.job_id}`)
          if (statusRes.code === 0) {
      ...
      exporting.value = false
      exportMsg.value = res.message || '启动失败'
    }
  } catch (e: any) {
...
const loadLoraModels = async () => {
  loading.value = true
  try {
    const res = await get<any[]>('/training/lora-models')
    if (res.code === 0) loraModels.value = res.data
