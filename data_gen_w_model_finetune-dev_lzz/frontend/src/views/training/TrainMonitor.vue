...
import { fetchTasks, fetchTask, cancelTask, resumeTask, connectTrainMonitor, fixTaskStatus, type GpuInfo, type LogEntry } from '@/api/training'
...
const connectToLatestTask = async () => {
  try {
    ...
    if (res.code === 0 && res.data.length > 0) {
      ...
      currentTask.value = running || res.data[0]  // tasks 已按时间倒序
...
const refreshTaskState = async () => {
  if (!currentTask.value) return
  try {
    const res = await fetchTask(currentTask.value.task_id)
    if (res.code === 0 && res.data) {
      ...
      Object.assign(taskStatus, {
        ...
        current_loss: res.data.current_loss ?? null,
        eta: res.data.eta ?? null,
      })
      if (res.data.loss_history) lossHistory.value = res.data.loss_history
    }
  } catch {}
}
...
const handleResume = async () => {
  if (!currentTask.value) return
  try {
    const res = await resumeTask(currentTask.value.task_id)
    if (res.code === 0) {
      ...
      lossHistory.value = []
      setTimeout(() => {
        currentTask.value!.task_id = res.data.task_id
        startWebSocket(res.data.task_id)
      }, 500)
    }
  } catch (e: any) {
