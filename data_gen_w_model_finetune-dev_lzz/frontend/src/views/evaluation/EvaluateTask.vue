    ...
    <div class="content-card">
      ...
      <el-table :data="evalTasks" stripe v-loading="loading">
        ...
        <el-table-column label="操作" width="160">
          <template #default="{ row }">
            <el-button v-if="row.status === 'completed'" link type="primary" size="small" @click="$router.push('/evaluation/report')">查看报告</el-button>
...
const loadEvalTasks = async () => {
  try {
    const res = await get<any[]>('/evaluation/tasks')
    if (res.code === 0) evalTasks.value = res.data
  } catch (e) { console.error(e) }
}

const deleteEval = async (taskId: string) => {
  try {
    await del(`/evaluation/tasks/${taskId}`)
...
const createEval = async () => {
  ...
  creating.value = true
  try {
    const res = await post('/evaluation/run', evalForm)
    if (res.code === 0) {
...
onMounted(async () => {
  const modelsRes = await get<{ local: any[]; merged: any[] }>('/training/verify-models')
  if (modelsRes.code === 0) availableModels.value = modelsRes.data
  const dsRes = await fetchDatasets()
  if (dsRes.code === 0) datasets.value = dsRes.data
  loadEvalTasks()
  // 仅当有运行中任务时才每5秒刷新
