...
class HuggingfaceEngine(BaseEngine):
    ...
    def _process_args(
        ...
        # ===== DEBUG: 打印原始消息 =====
        print("\n[DEBUG] Paired messages:")
        for m in paired_messages:
            print(f"  {m['role']}: {repr(m['content'])}")

        # ===== DEBUG: 打印完整 prompt 文本 =====
        prompt_ids, _ = template.encode_oneturn(tokenizer, paired_messages, system=None, tools=tools)
        try:
        ...
        print("\n[DEBUG] prompt last 10 tokens:", prompt_ids[-10:])
        print("[DEBUG] prompt last decode:", tokenizer.decode(prompt_ids[-10:]))
